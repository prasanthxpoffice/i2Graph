...
(function () {
  ...
  async function loadConfig() {
    ...
    _configPromise = (async () => {
      try {
        const inline =
          (typeof window !== "undefined" && window.LLM_CONFIG) || {};
        let fileCfg = {};
        try {
  ...
  function heuristicSuggest(headers, rows) {
    ...
    headers.forEach((h) => {
      const n = norm.get(h);
      if (!id && (n === "id" || n.endsWith("id") || n.includes("identifier")))
        id = h;
    ...
    if ((!ar || !en) && sample.length) {
      headers.forEach((h) => {
        ...
        if (!ar) {
          const arHits = vals.reduce(
            (a, v) => a + (containsArabic(v) ? 1 : 0),
            0
          );
  ...
  class HeuristicProvider {
    ...
    async suggestGroupsFromHeaders(headers, opts = {}) {
      ...
      headers.forEach((hId) => {
        ...
        const hEN = headers.find(
          (h) =>
            !used.has(h) &&
            (norm.get(h) === base + "nameen" || norm.get(h) === base + "en")
        );
        const hAR = headers.find(
          (h) =>
            !used.has(h) &&
            (norm.get(h) === base + "namear" || norm.get(h) === base + "ar")
        );
        if (hEN && hAR) {
          used.add(hId);
          used.add(hEN);
          used.add(hAR);
          groups.push({ id: hId, en: hEN, ar: hAR, idStrategy: "column" });
        }
      });
      ...
      headers.forEach((hId) => {
        ...
        if (!nId.endsWith("id")) return;
        const base = nId.slice(0, -2);
        const hName = headers.find(
          (h) => !used.has(h) && norm.get(h) === base + "name"
        );
        if (hName) {
          used.add(hId);
          used.add(hName);
          groups.push({ id: hId, en: hName, ar: hName, idStrategy: "column" });
        }
      });
      ...
      headers.forEach((h) => {
        if (forced.has(h) && !used.has(h)) {
          used.add(h);
          groups.push({ id: h, en: h, ar: h, idStrategy: "column" });
        }
      });

      const groupedCols = new Set(
        groups.flatMap((g) => [g.id, g.en, g.ar].filter(Boolean))
      );
      const ungrouped = headers
        .filter((h) => !groupedCols.has(h))
        .map((h) => ({
          column: h,
          reason: "no matching EN/AR pair or ID/NAME pattern",
        }));
      return {
        groups,
        ungrouped,
        reason: groups.length ? null : "Heuristic-only: no groups found.",
      };
    }
  }
  ...
  class ProxyProvider {
    ...
    async _postChat({ sys, user, temperature = 0.1 }) {
      try {
        window.Activity && window.Activity.start();
      } catch (_) {}
      ...
      try {
        ...
        try {
          window.Activity && window.Activity.end();
        } catch (_) {}
      }
    }
    ...
    async suggestRoles(headers, rows) {
      ...
      const sys =
        'You are a helpful CSV assistant. Given CSV headers and a few sample rows, pick the most likely columns for: ID (identifier), EN (English text), AR (Arabic text). Return JSON with keys {"id","en","ar"}, using exact header names when confident; else null.';
    ...
    async suggestGroupsFromHeaders(headers, opts = {}) {
      const { samples = [], forcedIncludes = [], minConfidence = 0.7 } = opts;
      const sys = [
      ...
      try {
        ...
        return {
          groups: safe,
          ungrouped: [],
          reason: "LLM unavailable; fallback applied",
        };
      }
    }
  ...
  const LLM = {
    ...
    async suggestGroupsFromHeaders(headers, opts = {}) {
      const p = this.getProvider();
      if (p.suggestGroupsFromHeaders)
        return p.suggestGroupsFromHeaders(headers, opts);
      return { groups: [], reason: "No provider available.", ungrouped: [] };
    },

    async suggestGroupNames(headers, groups, samples = []) {
      const p = this.getProvider();
      if (p.suggestGroupNames)
        return p.suggestGroupNames(headers, groups, samples);
      return [];
    },
  };
